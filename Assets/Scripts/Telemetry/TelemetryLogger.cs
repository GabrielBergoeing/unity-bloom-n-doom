...
/// Start/stop is driven externally (GameSession, around the match's actual Playing window).
///
...
public class TelemetryLogger : MonoBehaviour
{
    ...
    private void OnDestroy()
    {
        ...
        // changes scenes without ever going through the normal StopCapture() call.
        StopCapture();
    }
    ...
    // Tags both the filename and each row with player count, so runs with different
    // player counts (2/3/4-player stress tests, etc.) are easy to tell apart and group
    // together when comparing multiple exported CSVs afterwards.
    ...
    private string DeterminePath(string timestamp)
    {
        ...
        string playerTag = playerCount >= 0 ? $"_{playerCount}P" : "";
        captureName = $"match_{timestamp}{playerTag}";
    ...
    private string GetLatestRowData()
    {
        ...
        // NGO has no simple built-in equivalent to Mirror's NetworkTime.rtt - read the value
        // NetworkMetrics already computes every ping cycle instead of deriving our own.
        string latency = "N/A";
        if (NetworkMetrics.LastAvgRttMs > 0)
            latency = NetworkMetrics.LastAvgRttMs.ToString("F1", CultureInfo.InvariantCulture);
